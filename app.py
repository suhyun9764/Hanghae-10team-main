...
def create_memo():
    ...
    if member_id_to_query : 
        ...
        memo_data = Memo(main_text=text, edit_time=edit_time, member_id=member_id_to_query)
        if memo_data :
...
def member():
        member_id_to_query = session.get('member_id')
        if member_id_to_query : 
            member_list = AddressBook.query.filter_by(member_id = member_id_to_query)
...
def index():
    member_id_to_query = session.get('member_id')
    if member_id_to_query : 
        todos = Todo.query.filter_by(member_id=member_id_to_query).order_by(asc(Todo.date)).all()
